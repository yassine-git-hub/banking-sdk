using BankingSDK.Base.Stet.Contexts;
using BankingSDK.Base.Stet.Extensions;
using BankingSDK.Base.Stet.Models;
...
namespace BankingSDK.Base.Stet
{
    public abstract class BaseStetConnector : SdkBaseConnector , IBankingConnector
    {
        private StetUserContext _userContextLocal => (StetUserContext)_userContext;
        ...
        public BaseStetConnector(BankSettings settings, Uri productionUrl, Uri productionAuthUrl, ConnectorType connectorType):base(settings,connectorType)
        {
        ...
        {
            _userContext = new StetUserContext
            {
                UserId = userId
            };
        ...
        private async Task<List<AccountDto>> GetAccountsAsync(string token)
        {
            try
            {
                ...
                var url = $"/psd2/v1/accounts{(SdkApiSettings.IsSandbox ? (ConnectorType == ConnectorType.BE_HELLO_BANK ? "?brand=hb" : (ConnectorType == ConnectorType.BE_FINTRO ? "?brand=fintro" : "")) : "")}";
        ...
        {
            return new StetPagerContext(limit);
        }

        public IPagerContext RestorePagerContext(string json)
        {
            return JsonConvert.DeserializeObject<StetPagerContext>(json);
        }
        #endregion
